...
echo "***** Create Grafana Server START *****"
...
let "count++" # Create Grafana server
echo "$count) Create websesrver grafana..."
vm_name=$vm_name_grafana
image=$debian_image
disk_size=10
startup_script=$webserver_grafana_startup_script
...
echo "***** Create Grafana Server END *****"