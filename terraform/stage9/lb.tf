...
module "lb_grafana" {
    source = "../../../terraform_modules/http_load_balancer"
    backend_mig_name = module.mig_grafana.instance_group
    backend_service_name = var.grafana_backend_service_name
    health_checks = [module.grafana_health_check.self_link]
    url_map_name = var.grafana_url_map_name
    proxy_name = var.grafana_proxy_name
    lb_ipv4_name = var.grafana_lb_ipv4_name
    forwarding_rule_name = var.grafana_forwarding_rule_name
}